...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_fed_/_fed_/_sh/fed_heap_.sh"
...
fed_heap_() {
    ...
    d_name=$(dirname "/home/st"/rel_path_file_with_fn)
    if [ "man" == "$1" ]; then
    ...
    arr+=("dnf-automatic_")
    res+=("dnf-automatic_")
    arr+=("dnf_fedora_up_version_")
    res+=("dnf_fedora_up_version_")
    # arr+=("eeeeeeeee")
    # res+=("ttttttttt")
    # arr+=("eeeeeeeee")
    # res+=("ttttttttt")
