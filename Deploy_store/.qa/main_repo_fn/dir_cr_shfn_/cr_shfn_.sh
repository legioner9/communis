...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_cr_shfn_/cr_shfn_.sh"
...
MAIN: any stile path fn.sh with fn inside - cr_f_dir_insert_ --name \${filename_with_ext} --tml \${PATH_TML_DIR}/cr_shfn_/frime.tml --dir_insert  \${PATH_INSERT_DIR}/cr_shfn_
NAME: ${FNN}()
ARGS: 
\$1 filename_with_ext(e.c. tax.sh tax is function name)
CNTLS:
...
ERROR: return errno cr_f_dir_insert_ 
WARN: 
DEBUG:
EXAMP:${NORMAL}"
        return 0
    ...
    #? USE cr_shfn_() -echo -deb
    # path_name="$1"
    # if [ $(is_root_01 "$1") -ne 1 ]; then
    #     path_name=$PPWD/$path_name
    # fi
    unset_autoargs_cr_shfn_() {
        echo_cr_shfn_=0
        debug_cr_shfn_=0
        verbose_cr_shfn_=0
        develop1_cr_shfn_=0
        unset errmes_cr_shfn_
        unset outmes_cr_shfn_
        # any1_cr_shfn_=0
        qq_exit_cr_shfn_=0
        # unset any2_cr_shfn_
    }
    ...
    #?}
    #
    echo_deb_ ${echo_cr_shfn_} "cntl echo_deb_ mode in ${FNLOCK}"
    if [ -n ${debug_cr_shfn_} ] && [ ${debug_cr_shfn_} -eq 1 ]; then
        echo "DEBUG MODE in ${FNLOCK}"
        echo "\${errmes_cr_shfn_}: ${errmes_cr_shfn_}"
        echo "\${outmes_cr_shfn_}: ${outmes_cr_shfn_}"
    fi
    if [ -n ${develop1_cr_shfn_} ] && [ ${develop1_cr_shfn_} -eq 1 ]; then
        echo "develop1 MODE in ${FNLOCK}"
    fi
    if [ -n ${verbose_cr_shfn_} ] && [ ${verbose_cr_shfn_} -eq 1 ]; then
        echo "verbose MODE in ${FNLOCK}"
    fi
    exit_err_cr_shfn_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_cr_shfn_
    ...
    #! strongly return 1 -> exit_err_cr_shfn_ $1
    #! strongly return 0 -> exit_reg_cr_shfn_
    #! treat error for inner functions
    # >${FN_OUT_DIR}/com_cr_shfn_.res
    # COMMAND &>>${FN_OUT_DIR}/com_cr_shfn_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_cr_shfn_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_cr_shfn_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_cr_shfn_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
