...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_farg2e2_/farg2e2_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_farg2e2_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_farg2e2_
    ...
    # >${FN_OUT_DIR}/com_farg2e2_.res
    # COMMAND &>>${FN_OUT_DIR}/com_farg2e2_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_farg2e2_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_farg2e2_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_farg2e2_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    local item
    for item in ${ARGS[@]}; do
        if ! [ "${item:0:3}" == "--_" ] && ! [ "${item:0:2}" == "-_" ] && ! [ "${item:0:1}" == "^" ]; then
