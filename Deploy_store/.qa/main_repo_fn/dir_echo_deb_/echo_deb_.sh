...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_echo_deb_/echo_deb_.sh"
...
echo_deb_() {
        ...
        echo -e "${CYAN} ${FUNCNAME[0]}() help: 
            ...
            EXAMP: 
                ...
                fnname(){
                    \$echo_fnname=0
                    garg_ \$fnname \$@
                    echo_deb_ \$echo_fnname \"string for echo\"
                    }${NORMAL}"
        return 0
    fi
    ...
    *)
        ...
        # garg_ ${FUNCNAME[0]} $@
        if [ 1 -eq "$1" ]; then
            # echo -e "${BLUE}--- echo $2 ---${NORMAL}" #sistem info mesage
            echo -e "$2"
        elif [ 0 -ne "$1" ]; then
            plt_exit "echo_deb_ arg not define"
            return 1
        fi
