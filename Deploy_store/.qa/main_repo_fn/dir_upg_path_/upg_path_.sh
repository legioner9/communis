...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_upg_path_/upg_path_.sh"
...
MAIN: replase abspath with use env.sh name variables. overwrap (glob: \${path_file} path file [any style] ) exl_ --list \${PATH_EXL_DIR}/upg_path.exl 
NAME: ${FNN}()
...
EXAMP:${NORMAL}"
    ...
    #? USE upg_path_() -echo -deb
    # path_name="$1"
    # if [ $(is_root_01 "$1") -ne 1 ]; then
    #     path_name=$PPWD/$path_name
    # fi
    unset_autoargs_upg_path_() {
        echo_upg_path_=0
        debug_upg_path_=0
        verbose_upg_path_=0
        develop1_upg_path_=0
        unset errmes_upg_path_
        unset outmes_upg_path_
        # any1_upg_path_=0
        qq_exit_upg_path_=0
        # unset any2_upg_path_
    }
    unset_autoargs_upg_path_
    [[ 1 -eq ${verbose} ]] || {
        garg_ ${FNN} $@
    ...
    #?}
    #
    echo_deb_ ${echo_upg_path_} "cntl echo_deb_ mode in ${FNLOCK}"
    if [ -n ${debug_upg_path_} ] && [ ${debug_upg_path_} -eq 1 ]; then
        echo "DEBUG MODE in ${FNLOCK}"
        echo "\${errmes_upg_path_}: ${errmes_upg_path_}"
        echo "\${outmes_upg_path_}: ${outmes_upg_path_}"
    fi
    if [ -n ${develop1_upg_path_} ] && [ ${develop1_upg_path_} -eq 1 ]; then
        echo "develop1 MODE in ${FNLOCK}"
    fi
    if [ -n ${verbose_upg_path_} ] && [ ${verbose_upg_path_} -eq 1 ]; then
        echo "verbose MODE in ${FNLOCK}"
    fi
    exit_err_upg_path_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_upg_path_
    ...
    #! strongly return 1 -> exit_err_upg_path_ $1
    #! strongly return 0 -> exit_reg_upg_path_
    #! treat error for inner functions
    # >${FN_OUT_DIR}/com_upg_path_.res
    # COMMAND &>>${FN_OUT_DIR}/com_upg_path_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_upg_path_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_upg_path_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_upg_path_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    #     return 1
    # fi
    #! normalize arg_path -> absolut path
    # arg_path=$1
    ...
    # -----------------------------------------------------------------------------------------
    # ------------------------------------------
    path_file=$1 #! global var for upg_path.exl
    # exl_ --list "${PATH_EXL_DIR}"/upg_path.exl 1>/dev/null 1>/dev/null
    "${_ehh}" exl_ --list "${PATH_EXL_DIR}"/upg_path.exl
