...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_mm_fcr_/mm_fcr_.sh"
...
mm_fcr_() {
    ...
    #    wrp2_ rm -fr --_cxd ${PATH_MAIN_REPO_FN_DIR}/dir_fff_

        . ${FN_CONT_DIR}/_default/default_deb.sh
        # wrp2_ . --_xxf ${FN_CONT_DIR}/_default/default_deb.sh
...
MAIN: menu of wrapers f_f_arg_ --_name_fn fff_arg_ --_dir_fns ${tmp_fns}
NAME: ${FNN}()
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_fcr_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_fcr_
    ...
    # >${FN_OUT_DIR}/com_mm_fcr_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_fcr_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_fcr_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_fcr_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_fcr_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    res=()
    #{arr_res}
    arr+=("cr fonsh fn: f_f_arg_ 1 <name_fn>")
    res+=("f_f_arg_ --_name_fn $2 --_dir_fns ${FONSH_PATH}/actio --_flow 3")
    arr+=("cr plt fn: f_f_arg_ 2 <name_fn>")
    res+=("f_f_arg_ --_name_fn $2 --_dir_fns ${PLT_PATH}/actio --_flow 2")
    arr+=("cr communis fn: f_f_arg_ 3 <name_fn>")
    res+=("f_f_arg_ --_name_fn $2 --_dir_fns ${PATH_MAIN_REPO_FN_DIR} --_flow 1")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
