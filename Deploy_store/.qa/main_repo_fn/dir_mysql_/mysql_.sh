...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_mysql_/mysql_.sh"
...
mysql_() {
    ...
    *)
        ...
        arr=()
        res=()
        arr+=("mysql_install_fedora_")
        res+=("mysql_install_fedora_")
        arr+=("use [mysql] 'qweQWE123!@#'")
        res+=("sudo mysql -u 'root' -p")
        arr+=("install [mycli]")
        res+=("sudo pip3 install mycli")
        arr+=("use [mycli] 'qweQWE123!@#'")
        res+=("sudo mycli -u root -p 'qweQWE123!@#'")
