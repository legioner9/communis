...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_cr_cpp_h_/cr_cpp_h_.sh"
...
cr_cpp_h_() {
    ...
    *)
        ...
        echo -e "${BLUE}---Enter file name, will be (name).h ---${NORMAL}" #sistem info mesage
        read -r file_name
        up_file_name=$(echo $file_name | tr [:lower:] [:upper:])_H
        echo -e "${GREEN}\$up_file_name = $up_file_name${NORMAL}" #print variable
        qq_pause "up_file_name = $up_file_name continue? "

        echo -e "${ULINE}cp_h "${PATH_MAIN_REPO_FN_DIR}"/dir_cr_cpp_h_/_cr_cpp_h_/_tml/init.tml "$PPWD"/"$file_name".h${NORMAL}" #repit commands
        cp_h "${PATH_MAIN_REPO_FN_DIR}"/dir_cr_cpp_h_/_cr_cpp_h_/_tml/init.tml "$PPWD"/"$file_name".h

        sed -i 's/00000/'$up_file_name'/g' "$PPWD"/"$file_name".h
