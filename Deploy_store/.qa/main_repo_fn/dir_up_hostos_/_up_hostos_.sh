...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_up_hostos_/up_hostos_.sh"
...
up_hostos_() {
    ...
    if [ -f "/home/st/.hostos" ]; then
            HOSTOS=$(cat "/home/st/.hostos")
            if [ -n "$HOSTOS" ]; then
            ...
            read -p "/home/st/.HOSTOS not exist: Enter to exit or ^C to interrapt"
            exit 1
        fi
