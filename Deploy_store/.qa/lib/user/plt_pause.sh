...
filename="${COMMUNIS_PATH}/Deploy_store/.qa/lib/user/plt_pause.sh"
...
plt_pause() {
    ...
    echo -e "${RED}--------plt_pause: ${ARGS[@]} --------${NORMAL}" >&2 #sistem info mesage
    echo -e "${RED}^C - exit, ENTER - continue \nafter usually continue in parent thread ${NORMAL}" >&2   
    read 
    # read </dev/tty
