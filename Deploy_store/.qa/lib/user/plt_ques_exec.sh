#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/plt_ques_exec.sh"
...
plt_ques_exec() {
    ...
    local d_name=$(dirname ${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/plt_ques_exec.sh)
...
${NORMAL}"
    ...
    if [ "${yn}" == "y" ]; then
        echo -e "${HLIGHT}--- exec that :: ${ARGS[*]} ---${NORMAL}" #start files
        "${ARGS[@]}"
        return 0
    else
