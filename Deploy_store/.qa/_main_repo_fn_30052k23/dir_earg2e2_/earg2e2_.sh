...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_earg2e2_/earg2e2_.sh"
...
MAIN: echo [e]xecable [arg]s is not contane in \$(garg2e2_ "\${ARGS[\@]}")
NAME: ${FNN}()
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_earg2e2_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_earg2e2_
    ...
    # >${FN_OUT_DIR}/com_earg2e2_.res
    # COMMAND &>>${FN_OUT_DIR}/com_earg2e2_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_earg2e2_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_earg2e2_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_earg2e2_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    local arr_f_arg=$(farg2e2_ "${ARGS[@]}")

    arr_not_e_earg2e2_() {

        for item in ${g_args[@]}; do
            # echo "${item}"
            local g_val_arr=($(dlm_s "${item}" =))
            local g_val_arr1="${g_val_arr[1]}"

            if ! [[ ${g_val_arr1} == 1 ]]; then
                echo "${g_val_arr1}"
            fi
    ...
    local arr_not_e=$(arr_not_e_earg2e2_)

    #? arr_f_arg all not cntl_chars , arr_not_e all value for cntl_chars --> arr_f_arg - arr_not_e = arr_e_arg

    for item in ${arr_f_arg[@]}; do
        [[ " ${arr_not_e[*]} " =~ " ${item} " ]] || echo "${item}"
    done
