...
filename="$PATH_FN/dir_ques_arg_/ques_arg_.sh"
...
ques_arg_() {
    ...
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FUNCNAME[0]}() help: 
            MAIN: exec @* with entred single value
            NAME: ${FUNCNAME[0]}()
            ARGS:
            TAGS:
            FLOW:${NORMAL}"
    ...
    *)
        ...
        if_v_h "$1"
        # echo -e "${BLUE}---Exec $1 (y) ?---${NORMAL}" #sistem info mesage
        # unset y
        # read -r y
        # if [ "$y" = "y" ]; then
            ...
            read -r arg arg2
            if_v_s $arg
            echo -e "${BLUE}--- $@ $arg $arg2 ---${NORMAL}" #sistem info mesage
            $@ $arg $arg2
        # fi
