...
filename="$PATH_FN/dir_cr_f_dir_insert_/cr_f_dir_insert_.sh"
...
MAIN: in root dir create file from  --name, --dir_insert[, --tml]
NAME: ${FNN}()
ARGS: 
\$1
CNTLS:
...
EXAMP:${NORMAL}"
    ...
    for item in $(d2e ${dir_insert} "insert"); do
        ...
        echo -e "${BLUE}--- cr_f_ ${path_file} --fi0 ${name_insert}.insert --fr0 {${name_insert}} ---${NORMAL}" #sistem info mesage
        cr_f_ ${path_file} --fi0 ${dir_insert}/${name_insert}.insert --fr0 {${name_insert}}

    done

    cr_f_ ${path_file} --vi0 ${fn_name} --vr0 {fn_name}
    cr_f_ ${path_file} --vi0 ${root_path} --vr0 {root_path}
