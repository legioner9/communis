...
filename="${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_garg_/_garg_/_debug/pototype/use_2.sh"
    ...
    for item_i in $@; do
        ...
        if [ $flag -eq 1 ]; then
            ...
            if [ ${item_i:0:1} == "-" ]; then # param is flow_symbol - not a value
                qq_exit "$item_i not value - that flow_symbol"
            fi
            if [ -n $name_i ]; then
                eval $name_i=$item_i
                echo "$name_i = $item_i"
