...
filename="${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_fed_/_fed_/_sh/dnf-automatic_.sh"
...
dnf-automatic_() {
    ...
    d_name=$(dirname ${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_fed_/_fed_/_sh/dnf-automatic_.sh)
    if [ "man" == "$1" ]; then
    ...
    arr=()
    res=()
    arr+=("dnf-automatic_ man")
    res+=("dnf-automatic_ man")
    arr+=("install dnf-automatic")
    res+=("sudo dnf install dnf-automatic")
    arr+=("Активируем systemd-таймер")
    res+=("sudo systemctl enable dnf-automatic.timer")
    arr+=("edit /etc/dnf/automatic.conf")
    res+=("vi /etc/dnf/automatic.conf")
    # arr+=("eeeeeeeee")
    # res+=("ttttttttt")
    # arr+=("eeeeeeeee")
    # res+=("ttttttttt")
