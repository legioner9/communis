...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_mm_cr_all_/mm_cr_all_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_cr_all_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_cr_all_
    ...
    # >${FN_OUT_DIR}/com_mm_cr_all_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_cr_all_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_cr_all_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_cr_all_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_cr_all_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    res+=("echo bye)))")
    #{arr_res}
    arr+=("cr_sh_ simple .sh")
    res+=("cr_sh_ -h")
    arr+=("cr_shfn_ .sh with fn")
    res+=("cr_shfn_ -h")
    arr+=("cr_shgnr_ .sh generator")
    res+=("cr_shgnr_ -h")
    arr+=("cr_tst_dir_ simple tst dir")
    res+=("cr_tst_dir_ -h")
    arr+=("cr_tst_sys_ system tst dir")
    res+=("cr_tst_sys_ -h")
    arr+=("cr_d_ ?")
    res+=("cr_d_ -h")
    # arr+=("aaaaaaaaa")
