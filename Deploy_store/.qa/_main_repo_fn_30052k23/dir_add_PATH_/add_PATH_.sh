...
filename="$PATH_FN/dir_add_PATH_/add_PATH_.sh"
...
add_PATH_() {
    ...
    *)
        ...
        for path_for_PATH in $(f2e "$PATH_FN"/dir_"$FNN"/"$FNN".lst); do
            ...
            if [ 1 -eq "$flag" ]; then
                # echo -e "${BLUE}--- PATH=\$PATH:$path_for_PATH ---${NORMAL}" #sistem info mesage
                PATH="$path_for_PATH":$PATH
            fi
