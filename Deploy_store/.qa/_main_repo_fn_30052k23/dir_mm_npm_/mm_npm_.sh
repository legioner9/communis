...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_mm_npm_/mm_npm_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_npm_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_npm_
    ...
    # >${FN_OUT_DIR}/com_mm_npm_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_npm_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_npm_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_npm_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_npm_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    mm_npm_search_packet_in_npm_repo() {
        echo "Enter name for search"
        ques_arg_ npm search
    }
    ...
    res+=("echo bye)))")
    #{arr_res}
    arr+=("list global package")
    res+=("npm list -g --depth=0")
    arr+=("list root_dir package")
    res+=("npm list --depth=0")
    arr+=("list for update")
    res+=("npm outdated --depth=0")
    arr+=("update all global package")
    res+=("npm update -g")
    arr+=("update all root_dir package")
    res+=("npm update")
    arr+=("search packet in npm repo")
    res+=("mm_npm_search_packet_in_npm_repo")
    arr+=("update npm")
    res+=("npm install npm -g")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
