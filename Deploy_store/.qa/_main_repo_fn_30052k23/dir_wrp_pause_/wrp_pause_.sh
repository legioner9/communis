...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_wrp_pause_/wrp_pause_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_wrp_pause_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_wrp_pause_
    ...
    # >${FN_OUT_DIR}/com_wrp_pause_.res
    # COMMAND &>>${FN_OUT_DIR}/com_wrp_pause_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_wrp_pause_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_wrp_pause_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_wrp_pause_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    local errno=$?
    if [ "${errno}" -ne 0 ]; then
        plt_exit "Fail: errno=${errno} in ${ARGS[@]}"
    fi
