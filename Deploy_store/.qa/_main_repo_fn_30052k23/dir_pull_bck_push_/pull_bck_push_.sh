...
'
    ...
    if [ ! -d ${HOME}/Bare_communis/objects ]; then
        plt_pause " ${HOME}/Bare_communis/objects exist? "
        ...
        if [ 1 -eq "$count" ]; then
            ...
            if [ -d "$path_repo" ] || [ "git@github.com:legioner9/communis.git" == "$path_repo" ] || [ "git@gitlab.com:legioner9999/communis.git" == "$path_repo" ] || [ "ssh://legioner9@git.code.sf.net/p/communis/code" == "$path_repo" ]; then
                ...
                if [ "$name_repo" == "my_one" ] || [ "$name_repo" == "tosh_ext" ] || [ "$name_repo" == "tosh" ]; then
                    ...
                    # add flash file
                    # echo -e "${BLUE}--- cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/sh/_init_flash.sh $dir_repo ---${NORMAL}" #sistem info mesage
                    # cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/sh/_init_flash.sh $dir_repo
                    # echo -e "${BLUE}--- cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/sh/_rbl_communis.sh $dir_repo ---${NORMAL}" #sistem info mesage
                    # cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/sh/_rbl_communis.sh $dir_repo
                    # echo -e "${BLUE}--- cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/_exa_for_first_start/dnf_for_first_start.sh $dir_repo ---${NORMAL}" #sistem info mesage
                    # cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/_exa_for_first_start/dnf_for_first_start.sh $dir_repo
                    # echo -e "${BLUE}--- cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/_exa_for_first_start/prl.man $dir_repo ---${NORMAL}" #sistem info mesage
                    # cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/_exa_for_first_start/prl.man $dir_repo
                    # echo -e "${BLUE}--- rm -fr ${PATH_MY_ONE}/ALT_store/ ---${NORMAL}" #sistem info mesage
                    # rm -fr ${PATH_MY_ONE}/ALT_store/

                    # echo -e "${BLUE}--- cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/sh/_upd_all_alt.sh $dir_repo ---${NORMAL}" #sistem info mesage
                    # cp -fu ${PATH_COMMUNIS}/Deploy_store/plt_bd/sh/_upd_all_alt.sh $dir_repo

                    # echo -e "${BLUE}--- wrp_fifs2_ cp -rf ${PATH_COMMUNIS}/Store/ALT_store/ ${PATH_MY_ONE}/ -cd1 -cd2 ---${NORMAL}" #sistem info mesage

                    # wrp_fifs2_ cp -rf ${PATH_COMMUNIS}/Store/ALT_store/ ${PATH_MY_ONE}/ -cd1 -cd2 1>/dev/null

                    # echo -e "${BLUE}--- wrp_fifs2_ cp -rf ${PATH_COMMUNIS}/Deploy_store/plt_bd/main_store/sys_stores ${PATH_MY_ONE}/ -cd1 -cd2 ---${NORMAL}"

                    # wrp_fifs2_ cp -rf ${PATH_COMMUNIS}/Deploy_store/plt_bd/main_store/sys_stores ${PATH_MY_ONE}/ -cd1 -cd2 1>/dev/null
                    ...
                    # echo -e "${BLUE}--- wrp_fifs2_ cp ${tosh_disk}/up_to_communis/. ${PATH_COMMUNIS}/Store/up_from_tosh -cd1 ---${NORMAL}" #sistem info mesage

                    # wrp_fifs2_ cp -rfu ${tosh_disk}/up_to_communis/. ${PATH_COMMUNIS}/Store/up_from_tosh -cd1

                    # # up to tosh ${PATH_TOSHIBA_EXT}/

                    # store_communis="${PATH_COMMUNIS}/Store"
    ...
    #         echo -e "${ULINE}cp -ruT ${HOME}/Bare_communis/ ${PATH_MY_ONE}/COPY_bare/${NORMAL}" #repit commands
    #         cp -ruT ${HOME}/Bare_communis/ ${PATH_MY_ONE}/COPY_bare/
    #         # cd ${HOME}/cntr_bare/COPY_bare || plt_exit
    ...
    #         # cat "${PATH_COMMUNIS}/.date"

    #         # if ! diff -q "${HOME}/cntr_bare/COPY_bare/.date" "${PATH_COMMUNIS}/.date" >/dev/null; then
    #         #     cat "${HOME}/cntr_bare/COPY_bare/.date"
    #         #     cat "${PATH_COMMUNIS}/.date"
