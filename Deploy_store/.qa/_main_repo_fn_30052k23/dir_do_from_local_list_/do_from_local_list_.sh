...
MAIN: wrapper over l2exl_ --_quod ... --_exl ... using do_from_local_list_.list
NAME: ${FNN}()
...
required
    ...
    --_exl \${PATH_ARGS_DIR}/l2exl_/do_from_local_list/\${dilectus}
...
${NORMAL}"
    ...
    #? USE do_from_local_list_() -echo -deb
    local echo_
    local debug_
    local verbose_
    local develop1_
    ...
    local ptr_if_
    local ptr_sem_
    unset_autoargs_do_from_local_list_() {
        echo_=0
        debug_=0
        develop1_=0
        unset errmes_
    ...
    #{see_cntl}
    exit_err_do_from_local_list_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_do_from_local_list_
    ...
    # >${FN_OUT_DIR}/com_do_from_local_list_.res
    # COMMAND &>>${FN_OUT_DIR}/com_do_from_local_list_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_do_from_local_list_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_do_from_local_list_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_do_from_local_list_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    if ! wrp2_ file --_xxf ${list} >/dev/null; then
        plt_pause "Create empty ${list} ?"
        touch ${list}
    fi

    if ! wrp2_ file --_xxs ${PPWD}/do_from_local_list_.list >/dev/null; then
        plt_pause "File empty ${PPWD}/do_from_local_list_.list\n"
        return 1
    fi
    ...
    # ------------------------------------------
    # -----------------------------------------------------------------------------------------
    # altlinux_do_from_local_list_() {
    #     :
    # }
    # fedora_do_from_local_list_() {
    #     :
    # }
