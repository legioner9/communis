...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_mm_exlf_/mm_exlf_.sh"
...
MAIN: start exl_ with any files from ${PATH_EXL_DIR}/free_functions
NAME: ${FNN}()
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_exlf_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_exlf_
    ...
    # >${FN_OUT_DIR}/com_mm_exlf_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_exlf_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_exlf_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_exlf_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_exlf_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
