...
cr_man_h_unix_() {
    ...
    arr_fills_file=${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_cr_man_h_unix_/_cr_man_h_unix_/_tml/doc_file/arr_fills.tml
    ...
    *)
        ...
        cr_entety_cr_man_h_unix_() {
            ...
            for param in ${arr_params[@]}; do
                ...
                for sname in $(f2e2n_ctrm_ "$hdir"/"$hfile" "$param"); do
                    ...
                    if [ ! -d "$sroot" ]; then
                        # echo "mkdir_ $sroot"
                        echo -e "${BLUE}mkdir_ $sroot${NORMAL}" #sistem info mesage
                    ...
                    if [ ! -f "$spath" ]; then
                        ...
                        cp ${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_cr_man_h_unix_/_cr_man_h_unix_/_tml/doc_file/file_entety.tml "$spath"
                        ...
                        sdir2="$(basename "$sdir")"
                        # echo -e "${GREEN}\$sdir2 = $sdir2${NORMAL}" #print variable

                        if [ "$sdir2" == "anys" ]; then
                            sname2="$sname""(7)"
                            sed -i 's|00000|'"$sname2"'|g' "$spath"
                            sed -i 's|11111|'"$hfile"'|g' "$spath"
                            sed -i 's|22222|'"$hfile_body"'|g' "$spath"
                        else
                            sed -i 's|00000|'"$sname"'|g' "$spath"
                        ...
                        # echo see: ${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_c_/_c_/_man/nbase/MANC/contents/"$sname".cnt
                        if [ -f ${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_c_/_c_/_man/nbase/MANC/contents/"$sname".cnt ]; then
        ...
        for hfile in $(d2e "$hdir"); do
            ...
            #     f2f_mt "$fill_dir"/"$hfile_body"/"$fill" "$hname" "<<""$fill"">>"
            ...
            #     if [ "$body_cnt" == "$hfile_body" ] || [ "$body_cnt" == "$hfile_body"_1 ]; then
            #         # echo -e "${GREEN}\$body_cnt = $body_cnt${NORMAL}" #print variable
            #         f2f_mt $PPWD/contents/$cnt $hname "<<ISU>>"
            #     fi
