...
/*#*/ int close_с(int fildes, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        ...
        //     break;
        // case EINTR: // во время выполнения вызова был получен сигнал, который прервал выполнение вызова
        //     break;
        // case EIO: // свидетельствует об ошибке ввода-вывода (*только Linux)
        //     break;
...
/*crypt*/
/*dup*/
/*#*/ int dup_с(int oldfd, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    /* SINGL SECTION START -----------------------------------------*/
    if (st__if & STIF_ONE)
    {
...
/*pwrite*/
/*read*/
/*#*/ ssize_t read_с(int fildes, void *buf, size_t nbyte, const char *file, const int line, int st__mo, int st__if, int st__re)
{
