...
/*#*/ int getsockname_с(int socket, struct sockaddr *restrict address, socklen_t *restrict address_len, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        int a = getsockname(socket, address, address_len);
        if (-1 == a)
        {
            if (st__mo & STMO_PRN)
            {
                fprintf(stderr, "failed in file %s at line # %d\n", file, line);
                fprintf(stderr, "from getsockname_с socket = %d, errno = %d\n", socket, errno);
                perror("Error is: ");
                fflush(stdout);
            }
            if (st__re & STRE_MNS)
...
/*#*/ ssize_t recvfrom_c(int socket, void *restrict buffer, size_t length, int flags, struct sockaddr * address, socklen_t *restrict address_len, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        ssize_t a = recvfrom(socket, buffer, length, flags, address, address_len);
        if (-1 == a)
        {
            if (st__mo & STMO_PRN)
            {
                fprintf(stderr, "failed in file %s at line # %d\n", file, line);
                fprintf(stderr, "from recvfrom_с socket = %d, errno = %d\n", socket, errno);
                perror("Error is: ");
                fflush(stdout);
            }
            if (st__re & STRE_MNS)
...
/*#*/ int socket_с(int domain, int type, int protocol, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        ...
        if (-1 == a)
        {
            ...
            // file://${COMMUNIS_PATH}/Deploy_store/.qa/main_repo_fn/dir_c_/_c_/_man/nbase/_Client_Server/Full_gd/1/2_Полное руководство по сетевом.pdf
            // switch (errno)
            // {
