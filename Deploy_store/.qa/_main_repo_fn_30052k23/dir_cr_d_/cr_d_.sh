...
filename="$PATH_FN/dir_cr_d_/cr_d_.sh"
...
cr_d_() {
    ...
    *)
        ...
        echo_deb_ $echo_cr_d_ "cntl echo_deb_ mode in ${FUNCNAME[0]}"
        if [ $deb_cr_d_ -eq 1 ]; then
            echo "DEBUG MODE in ${FUNCNAME[0]}"
        fi

        full_path=$(upd_to_root_ "$PPWD" "$1")
        echo_deb_ $echo_cr_d_ $full_path

        # if [ -n "$1"];then
        #         file_ds=$1
        #         if [ "$(is_root_01 $file_ds)" -eq 0 ] ; then
        #         file_ds="$PPWD"/"$file_ds"
        #         fi
        # fi
        #!function body======================
        #!function body===============================================
