...
filename="$PATH_FN/dir_correct_/correct_.sh"
...
correct_() {
    ...
    *)
        ...
        arr+=("english PHRASE")
        res+=("echo file://${PATH_COMMUNIS}/Store/English_info/dictionary_phrase.man")
        arr+=(".tst PATH")
        res+=("echo file://${PATH_CONFIG_DIR}/.tst")
        arr+=("collect + editall_tst_files.list")
        res+=("")
        #5
        arr+=("C plt libs (_c_/_lib)")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_c_/_c_/_lib")
        arr+=("_dnf_ boot menu")
        res+=("echo file://${PATH_COMMUNIS}/Deploy_store/.qa/menus/_set_menu/_mm_flow_/_mm_0_start_sys_/dnf_.sh")
        arr+=("own all cr_pj_c_simp")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_cr_pj_c_simp_/_cr_pj_c_simp_/")
        arr+=("info_c in cr_pj_c_simp")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_cr_pj_c_simp_/_cr_pj_c_simp_/_tml/tml_dir/info_c")
        arr+=(".canons/.bfnlist.md")
        res+=("echo file://${PATH_CANONS_DIR}/.bfnlist.md")
        #10
        arr+=("manclang_tailpath hint_")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_hint_/_hint_/_lists/manclang_tailpath.list")
        arr+=("env.sh")
        res+=("echo file://${PATH_CONFIG_DIR}/env.sh")
        arr+=("deploy_sistem functions ")
        res+=("echo file://${PATH_COMMUNIS}/Deploy_store/deploy_sistem")
        arr+=(".list2 for bd_communis_")
        res+=("echo file://${PATH_COMMUNIS}/Deploy_store/plt_bd/list2")
        arr+=("_tmls to sys")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/_tmls/")
        #15
        arr+=("prl.man")
        res+=("echo file://${PATH_COMMUNIS}/Deploy_store/plt_bd/_exa_for_first_start/prl.man")
        arr+=(".canons/.cfnlist.md")
        res+=("echo file://${PATH_CANONS_DIR}/.cfnlist.md")
        arr+=("latina/keywords.man")
        res+=("echo file://${PATH_COMMUNIS}/Store/latina/keywords.man")
        arr+=("add_PATH_.lst")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_add_PATH_/add_PATH_.lst")
        arr+=("CEXEC_PROJECT_DIR")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_c_/_c_/_exec")
        #20
        arr+=("PATH_GIT")
        res+=("echo file://${PATH_COMMUNIS}")
        arr+=("PATH_TST_FILE")
        res+=("echo file://${PATH_CONFIG_DIR}/.tst")
        arr+=("PATH_ENV_FILE")
        res+=("echo file://${PATH_CONFIG_DIR}/env.sh")
        arr+=("rbl All MAN .list")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_mancc_/mancc_.list")
        arr+=("dir example c files")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_c_/_c_/_exam_files")
        #25
        ...
        arr+=("vim tutorial")
        res+=("echo file://${PATH_COMMUNIS}/Store/vim/Vim_учебник_ALT_Linux_Wiki.pdf")
        arr+=(".tml for cr_f_")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_cr_f_/_cr_f_/_tml")
        #30
        arr+=("_apt_alt_ boot menu")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_apt_alt_/apt_alt_.sh")
        arr+=("fonsh fn")
        res+=("echo file://${PATH_CONFIG_DIR}/fonsh_fn.man")
        arr+=("add PATH .list")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_add_PATH_/add_PATH_.lst")
        arr+=(".canons/.gitfnlist.md")
        res+=("echo file://${PATH_CANONS_DIR}/.gitfnlist.md")

        arr+=("define files to mm_dogit_")
        res+=("echo file://${PATH_MAIN_REPO_FN_DIR}/dir_mm_dogit_/_mm_dogit_/_fn/mm_dogit_define_files_dirs.sh")

        arr+=(".canons/.pltopus.md")
        res+=("echo file://${PATH_CANONS_DIR}/.pltopus.md")
        arr+=("4 CLR list")
        res+=("echo file://${PATH_LISTS_DIR}/git/hsc_alt/curr_list_repo_CLR.list")
        arr+=("5 MBL list")
        res+=("echo file://${PATH_LISTS_DIR}/git/hsc_alt/main_bcp_local_MBL.list")
        arr+=("12 OBL list")
