...
filename="$PATH_OS/.qa/sis_git/bck_git.sh"
...
q_bck() { # $repo_path=$repo_path $1=$did
    ...
    echo -e "${ULINE}cp -rf "$PATH_OS/.os" "$repo_path/_repo/communis/Sis_USER/$1/"${NORMAL}" #repit commands
    cp -rf "$PATH_OS/.os" "$repo_path/_repo/communis/Deploy_store/"
    ...
    echo -e "${ULINE}cp -rf "$PATH_OS/.qa" "$repo_path/_repo/communis/Sis_USER/$1/"${NORMAL}" #repit commands
    cp -rf "$PATH_OS/.qa" "$repo_path/_repo/communis/Deploy_store"
