...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_l2exl_/l2exl_.sh"
...
CNTLS:
required 
    --_quod list for exl_ (queue)
    --_exl .exl file (handler)
...
${FNN} _deb, ${FNN} _mdeb
EXAMP:
    ${FNN} --_quod \${PATH_ARGS_DIR}/l2exl_/test_1/test.plist --_exl \${PATH_ARGS_DIR}/l2exl_/test_1/test.exl
${NORMAL}"
        return 0
    fi
    #{cntl_help}
    ...
    #{see_cntl}
    exit_err_l2exl_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_l2exl_
    ...
    # >${FN_OUT_DIR}/com_l2exl_.res
    # COMMAND &>>${FN_OUT_DIR}/com_l2exl_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_l2exl_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_l2exl_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_l2exl_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    [[ 1 -eq ${verbose} ]] || echo -e "${GREEN}\$PPWD = $PPWD${NORMAL}" #print variable
    quod_=$(abs_path $PPWD "quod_")
    exl_=$(abs_path $PPWD "exl_")

    est_ ${quod_} -f
    est_ ${exl_} -f

    local l2exl_item
    for l2exl_item in $(f22e_ ${quod_}); do
        #? l2exl_item for exl_ proc
        exl_ --list ${exl_}
    done
