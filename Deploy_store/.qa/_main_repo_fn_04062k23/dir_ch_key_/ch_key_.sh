...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_ch_key_/ch_key_.sh"
...
ch_key_() {
    ...
    *)
        ...
        "${arr[0]}")
            echo -e "${CYAN}---${arr[0]}---${NORMAL}" #started functions
            ch_from_setkey_
            _ch_key_="$_ch_from_setkey_"
            qq_pause "search by KEY = $_ch_key_ [from - ${#_g_s_m_[@]}] ?"

            ;;

        "${arr[1]}")
            echo -e "${CYAN}---${arr[1]}---${NORMAL}"                                           #started functions
            echo -e "${BLUE}--- enter you KEY by search [from - ${#_g_s_m_[@]}] : ---${NORMAL}" #sistem info mesage
            read -r _ch_key_
            qq_pause "search by KEY = $_ch_key_ [from - ${#_g_s_m_[@]}] ?"
