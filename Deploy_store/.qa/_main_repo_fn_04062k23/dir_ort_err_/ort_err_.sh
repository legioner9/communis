...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_ort_err_/ort_err_.sh"
...
MAIN: check --ext files from --list directories
NAME: ${FNN}()
...
CNTLS:
required 
    --ext extends file wich exec tst
    --list file with start search directories
optional 
...
${NORMAL}"
    ...
    exit_err_ort_err_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_ort_err_
    ...
    # >${FN_OUT_DIR}/com_ort_err_.res
    # COMMAND &>>${FN_OUT_DIR}/com_ort_err_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_ort_err_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_ort_err_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_ort_err_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
