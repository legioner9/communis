...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_is_gclean_/is_gclean_.sh"
...
MAIN: in root dir --name --tml return 0 if nothing to add, else return 1
NAME: ${FUNCNAME[0]}()
ARGS: 
\$1
CNTLS:
...
EXAMP:${NORMAL}"
    ...
    #? USE is_gclean_() -echo -deb
    # path_name="$1"
    # if [ $(is_root_01 "$1") -ne 1 ]; then
    #     path_name=$PPWD/$path_name
    # fi
    unset_autoargs_is_gclean_() {
        echo_is_gclean_=0
        debug_is_gclean_=0
        unset errmes_is_gclean_
        unset outmes_is_gclean_

        unset name_is_gclean_
        unset tml_is_gclean_

        # any1_is_gclean_=0
        qq_exit_is_gclean_=0
        # unset any2_is_gclean_
    }
    ...
    #?}
    #
    echo_deb_ ${echo_is_gclean_} "cntl echo_deb_ mode in ${FNLOCK}"
    if [ ${debug_is_gclean_} -eq 1 ]; then
        echo "DEBUG MODE in ${FNLOCK}"
        echo "\$errmes_is_gclean_: "$errmes_is_gclean_""
        echo "\$outmes_is_gclean_: "$outmes_is_gclean_""
    fi
    exit_is_gclean_() {
        unset_autoargs_is_gclean_
    ...
    #! MAST BE AFTER exit_is_gclean_: return 1
    # -----------------------------------------------------------------------------------------
    # ------------------------------------------

    #* >>>>>>>>>> create variable $name_repo to use at ${tml_gclone_} enterpritation file >>>>>>>>>>
    name_repo=${name_is_gclean_}
    #* enterpritation file - resive global variables containg at file

    if [ ${debug_is_gclean_} -eq 1 ]; then
        exl_ --list "${tml_is_gclean_}" -qq_exit
    else
        exl_ --list "${tml_is_gclean_}" -qq_exit 1>/dev/null
    fi
    ...
    if [ ${debug_is_gclean_} -eq 1 ]; then
        agv2e nv_from_tml_ "${tml_is_gclean_}"
    else
        agv2e nv_from_tml_ "${tml_is_gclean_}" 1>/dev/null
    fi
    #* <<<<<<<<<< create variable $name_repo to use at ${tml_gclone_} enterpritation file <<<<<<<<<<

    #* >>>>>>>>>> recive name_var for pair remote-addres
    echo_deb_ "${echo_is_gclean_}" "${BLUE}--- wrp_fifs1_ cd ${curr_repo_dir} -d1 ---${NORMAL}" #sistem info mesage

    wrp_fifs1_ cd ${curr_repo_dir} -d1

    match_str="Untracked files"

    if is_ginname_ --tml "${tml_is_gclean_}" --name ${name_is_gclean_}; then
        ...
        exl_pause "is_ginname_ error: name not ${name_is_gclean_}"
    fi
