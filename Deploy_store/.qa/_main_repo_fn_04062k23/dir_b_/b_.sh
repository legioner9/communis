...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_b_/b_.sh"
...
b_() {
    ...
    *)
        ...
        arr+=("[a_] A_point list")
        res+=("a_")
        arr+=("THIS [b_] start A_point [plt functions]")
        res+=("b_")
        arr+=("[i_] start hot functions ")
        res+=("i_")
        arr+=("[ca_] start [learn_explore functions]")
        res+=("ca_")
        arr+=("[s_s_] search main_fn by tag")
        res+=("s_s_")
        arr+=("[s_w_] search man info by word")
        res+=("s_w_")
        arr+=("[f_f_] create main_fn")
        res+=("f_f_")
        arr+=("[qqq_enter] git framework")
        res+=("qqq_enter")
        arr+=("[spec_ _0] spec_plt.man lexicon_plt.man")
        res+=("spec_ _0")
        arr+=("[cr_0] start_intreract   cr_dir_from_file_ , prepeare that")
        res+=("cr_0")
        arr+=("[cpp_] start g++ in fis_dir")
        res+=("cpp_")
        # arr+=("c")
        # res+=("cp")
        # arr+=("c")
        ...
        "${arr[5]}")
            ...
            echo -e "${BLUE}---Enter arg for f_f_ ---${NORMAL}" #sistem info mesage
            read -r arg
            ${res[5]} "$arg"
            ;;
