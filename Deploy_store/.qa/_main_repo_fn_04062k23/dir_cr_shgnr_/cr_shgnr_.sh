...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_cr_shgnr_/cr_shgnr_.sh"
...
MAIN: cr_sh_ with insert dir \${PATH_INSERT_DIR}/cr_shgnr_/\${number_flow} (1|2|...)
NAME: ${FNN}()
...
\$1 number_flow
    1 - tst dir
    2 - tst sys
\$2 inname_fale (_gnr_<inname_fale>.sh) 
CNTLS:
required
...
${FNN} _deb, ${FNN} _mdeb
EXAMP:
${FNN} <numflow> <name_inside> -> cr _gnr_<name_inside>.sh
${NORMAL}"
        return 0
    fi
    #{cntl_help}
    ...
    #{see_cntl}
    exit_err_cr_shgnr_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_cr_shgnr_
    ...
    # >${FN_OUT_DIR}/com_cr_shgnr_.res
    # COMMAND &>>${FN_OUT_DIR}/com_cr_shgnr_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_cr_shgnr_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_cr_shgnr_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_cr_shgnr_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    local number_flow=${ARGS[0]}
    local name_file=_gnr_${ARGS[1]}.sh
