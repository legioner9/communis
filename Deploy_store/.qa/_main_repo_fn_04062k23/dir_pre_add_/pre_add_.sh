...
filename="$PATH_FN/dir_pre_add_/pre_add_.sh"
...
pre_add_() {
    ...
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FUNCNAME[0]}() help: 
            MAIN: gefine in repo, use in pre-commit as function before exec [git add .]
            NAME: ${FUNCNAME[0]}()
            ARGS: 
            FLOW:
            - present in repo fs
            - up to memory in precommit
            - prepeare work dir fefore [git add .] : zip, cp, rm, exec libs ect...
            - 
