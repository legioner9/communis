...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_cr_tst_dir_/cr_tst_dir_.sh"
...
\$1
CNTLS:
    required: --name NAME (for /NAME_tst/)  --ext EXT (for exec._EXT)
    optional: 
...
${NORMAL}"
    ...
    exit_err_cr_tst_dir_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_cr_tst_dir_
    ...
    # >${FN_OUT_DIR}/com_cr_tst_dir_.res
    # COMMAND &>>${FN_OUT_DIR}/com_cr_tst_dir_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_cr_tst_dir_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_cr_tst_dir_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_cr_tst_dir_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    #? ${g_args[@]}: name_dir=ex1 ext=extst verbose=1
