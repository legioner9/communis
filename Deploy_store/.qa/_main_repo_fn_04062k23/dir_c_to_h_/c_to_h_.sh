...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_c_to_h_/c_to_h_.sh"
...
c_to_h_() {
    ...
    if [ "-u" == "$1" ]; then
        for item in "$(cat ${PATH_MAIN_REPO_FN_DIR}/dir_$FNN/all_c_lib.lst)"; do
            c_to_h_lib_dir_ "${PATH_MAIN_REPO_FN_DIR}/$item"
        done
        return 0
    fi
