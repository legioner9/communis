...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_nar_/nar_.sh"
...
MAIN: menu narratio regula, action (install plt from usb, exl_) 
NAME: ${FNN}()
...
${NORMAL}"
    ...
    # >${FN_OUT_DIR}/com_nar_.res
    # COMMAND &>>${FN_OUT_DIR}/com_nar_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_nar_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_nar_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_nar_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    nar_menu() { # $1 fix dir
        local dir_select="${PATH_MAN_DIR}/nar/$1"
        local delectus=""
        PS3="eligendi actiones $1: "

        select delectus in $(ls ${dir_select}); do
            # echo ${delectus}
            break
        done

        echo -e "${RED}--- start cat ${dir_select}/${delectus} ---${NORMAL}" #sistem info mesage
        cat ${dir_select}/${delectus}
        echo -e "\n${RED}--- end cat ${dir_select}/${delectus} ---${NORMAL}" #sistem info mesaget 1

    }

    # nar_actio_menu() {
    #     local dir_select="${PATH_MAN_DIR}/nar/actio"
    #     local delectus=""
    #     PS3="eligendi actiones actio: "
    ...
    #     echo -e "${RED}--- start cat ${dir_select}/${delectus} ---${NORMAL}" #sistem info mesage
    #     cat ${dir_select}/${delectus}
    #     echo -e "${RED}--- end cat ${dir_select}/${delectus} ---${NORMAL}" #sistem info mesaget 1
    # }

    echo -e "${BLUE}--- exec ${FNN} (num_menu) ---${NORMAL}" #started functions
    arr=()
    res=()
    local dir_dir="${PATH_MAN_DIR}/nar"
    local item_dir=""
    for item_dir in $(ls ${dir_dir}); do
        if [ -d ${dir_dir}/${item_dir} ]; then
            arr+=("${item_dir}")
            res+=("nar_menu ${item_dir}")
        fi
