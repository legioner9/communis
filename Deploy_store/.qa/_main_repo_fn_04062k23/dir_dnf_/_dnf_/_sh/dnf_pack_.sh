...
filename="${COMMUNIS_PATH}/Deploy_store/.qa/menus/_set_menu/_mm_flow_/_mm_0_start_sys_/_dnf_/dnf_pack_.sh"
...
dnf_pack_() {
    ...
    d_name=$(dirname ${COMMUNIS_PATH}/Deploy_store/.qa/menus/_set_menu/_mm_flow_/_mm_0_start_sys_/_dnf_/dnf_pack_.sh)
    if [ "man" == "$1" ]; then
    ...
    arr=()
    res=()
    arr+=("Go to dir with .repo (yum)")
    res+=("cd /etc/yum.repos.d/")
    arr+=("inst RPM Fusion")
    res+=("sudo dnf install --nogpgcheck https://download1.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://download1.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm")
    arr+=("uninst RPM Fusion")
    res+=("sudo dnf remove rpmfusion-free-release rpmfusion-nonfree-release")
    arr+=("inst Negativo17")
    res+=("sudo dnf config-manager --add-repo=http://negativo17.org/repos/fedora-negativo17.repo")
    arr+=("uninst Negativo17")
    res+=("sudo rm -f /etc/yum.repos.d/fedora-negativo17.repo")
    arr+=("inst VirtualBox")
    res+=("sudo dnf config-manager --add-repo=https://github.com/RussianFedora/fedora-extra-repos/raw/master/virtualbox.repo")
    arr+=("uninst VirtualBox")
    res+=("sudo rm -f /etc/yum.repos.d/virtualbox.repo")
    arr+=("inst VSCode")
    res+=("sudo dnf config-manager --add-repo=https://github.com/RussianFedora/fedora-extra-repos/raw/master/code.repo")
    arr+=("uninst VSCode")
    res+=("sudo rm -f /etc/yum.repos.d/code.repo")
    arr+=("inst Yandex.Browser")
    res+=("sudo dnf config-manager --add-repo=https://github.com/RussianFedora/fedora-extra-repos/raw/master/yandex-browser-beta.repo")
    arr+=("uninst Yandex.Browser")
    res+=("sudo rm -f /etc/yum.repos.d/yandex-browser-beta.repo")
    arr+=("inst Dropbox")
    res+=("sudo dnf config-manager --add-repo=https://github.com/RussianFedora/fedora-extra-repos/raw/master/dropbox.repo")
    arr+=("uninst Dropbox")
    res+=("sudo rm -f /etc/yum.repos.d/dropbox.repo")
    # arr+=("eeeeeeeee")
    # res+=("ttttttttt")
    # arr+=("eeeeeeeee")
    # res+=("ttttttttt")
