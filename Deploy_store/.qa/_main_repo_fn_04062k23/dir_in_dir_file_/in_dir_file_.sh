...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_in_dir_file_/in_dir_file_.sh"
...
in_dir_file_() {
    ...
    *)
        ...
        in_dir_file_L_1() {
            ...
            echo -e "${BLUE}---Exec mkdir_h (y) ?---${NORMAL}" #sistem info mesage
            unset y
            read -r y
            if [ "$y" = "y" ]; then

                echo -e "${BLUE}---Enter arg for exec mkdir_h ---${NORMAL}" #sistem info mesage
                unset arg
                read -r arg arg2
                if_v_s $arg
                mkdir_h $arg $arg2
