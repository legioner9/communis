...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_gh_/gh_.sh"
...
EXAMP:${NORMAL}"
    ...
    #?}
    #
    echo_deb_ ${echo_gh_} "cntl echo_deb_ mode in ${FNLOCK}"
    if [ ${debug_gh_} -eq 1 ]; then
        echo "DEBUG MODE in ${FNLOCK}"
        echo "\${errmes_gh_}: ${errmes_gh_}"
        echo "\${outmes_gh_}: ${outmes_gh_}"
    fi
    if [ ${develop1_gh_} -eq 1 ]; then
        echo "develop1 MODE in ${FNLOCK}"
    fi
    if [ ${verbose_gh_} -eq 1 ]; then
        echo "verbose MODE in ${FNLOCK}"
    fi
    exit_err_gh_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_gh_
    ...
    #! strongly return 1 -> exit_err_gh_ $1
    #! strongly return 0 -> exit_reg_gh_
    #! treat error for inner functions
    # >${FN_OUT_DIR}/com_gh_.res
    # COMMAND &>>${FN_OUT_DIR}/com_gh_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_gh_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_gh_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_gh_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    gh_install_rpm_2_22_0() {
        # https://github.com/cli/cli/releases
        # https://cloudaffaire.com/how-to-install-and-configure-github-cli-gh/
        wrp_fifs1_ cd ${COMMUNIS_PATH}/Store/RPM
        plt_pause "wget gh_2.22.0_linux_386.rpm in $(pwd)?"
        if ! [ -f gh_2.22.0_linux_386.rpm ]; then
            wget https://github.com/cli/cli/releases/download/v2.22.0/gh_2.22.0_linux_386.rpm
        else
            echo -e "${BLUE}--- gh_2.22.0_linux_386.rpm allready exist ---${NORMAL}" #sistem info mesage
        fi

        sudo rpm -i gh_2.22.0_linux_386.rpm
    ...
    arr+=("leave menu")
    res+=("echo bye)))")
    # arr+=("install rpm gh_2.22.0_linux_386.rpm")
    # res+=("gh_inst_2_22_0")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
    # arr+=("aaaaaaaaa")
