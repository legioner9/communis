...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_cr_bin_/cr_bin_.sh"
...
cr_bin_() {
    ...
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FUNCNAME[0]}() help: 
            MAIN: create cr_pj_c_simp_() in ${PATH_MAIN_REPO_FN_DIR}/dir_c_/_c_/_exec and cd to that dir
            NAME: ${FUNCNAME[0]}()
            ARGS:
            CNTL: [, -echo, -debug ...]
            TAGS:
    ...
    exec_dir=${PATH_MAIN_REPO_FN_DIR}/dir_c_/_c_/_exec
