...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_wrp_fifs2_/wrp_fifs2_.sh"
...
wrp_fifs2_() {
    ...
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FUNCNAME[0]}() help: 
            ...
            -d dir
            DEBUG:
            . '${PATH_MAIN_REPO_FN_DIR}/dir_cp__/cp__tst/exec.tst'
    ...
    *)
        ...
        if [ -n "${!f2}" ] && [ "${!f2}" -eq 1 ]; then
            if ! [ -f $dist ]; then
                plt_pause "$dist not exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
                echo "$dist not exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO" 1>&2
                return 1
            fi
        fi
        ...
        if [ -n "${!nf2}" ] && [ "${!nf2}" -eq 1 ]; then
            if [ -f $dist ]; then
                plt_pause "$dist exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
                echo "$dist exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO" 1>&2
                return 1
            fi
        fi
        ...
        if [ -n "${!d2}" ] && [ "${!d2}" -eq 1 ]; then
            if ! [ -d $dist ]; then
                plt_pause "$dist not exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
                echo "$dist not exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO" 1>&2
                return 1
            fi
        fi
        ...
        if [ -n "${!nd2}" ] && [ "${!nd2}" -eq 1 ]; then
            if [ -d $dist ]; then
                plt_pause "$dist exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
                echo "$dist exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO" 1>&2
                return 1
            fi
        fi
        ...
        if [ -n ${!e2} ] && [ ${!e2} -eq 1 ]; then
            if ! [ -e $dist ]; then
                plt_pause "$dist not exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
                echo "$dist not exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO" 1>&2
                return 1
            fi
        fi
        ...
        if [ -n ${!ne2} ] && [ ${!ne2} -eq 1 ]; then
            if [ -e $dist ]; then
                plt_pause "$dist exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
                echo "$dist exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO" 1>&2
                return 1
            fi
        fi
