...
wrp_fifs2_() {
    ...
    *)
        ...
        local _echo_=echo_"$bfn"__
        local _echo_=0

        #o

        local co12=co12_"$bfn"__
        local co12_"$bfn"__=0
        innrer_arg+=("$co12=1")

        local co21=co21_"$bfn"__
        local co21_"$bfn"__=0
        innrer_arg+=("$co21=1")
        ...
        local _edeb=echo_$1__
        local echo_$1__=0
        local _debag=deb_$1__
        local deb_$1__=0

        enter_args=($(garg_ "$1"__ "${ARGS[@]}"))

        garg_ "$1"__ "${ARGS[@]}"

        echo_deb_ ${!_edeb} "cntl echo_deb_ mode in ${FUNCNAME[0]}"
        if [ ${!_debag} -eq 1 ]; then
        ...
        if [ -n "${!f1}" ] && [ "${!f1}" -eq 1 ]; then
            if ! [ -f $src ]; then
                qq_exit "$src not exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!nf1} ] && [ ${!nf1} -eq 1 ]; then
            if [ -f $src ]; then
                qq_exit "$src exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!d1} ] && [ ${!d1} -eq 1 ]; then
            if ! [ -d $src ]; then
                qq_exit "$src not exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!nd1} ] && [ ${!nd1} -eq 1 ]; then
            if [ -d $src ]; then
                qq_exit "$src exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!e1} ] && [ ${!e1} -eq 1 ]; then
            if ! [ -e $src ]; then
                qq_exit "$src not exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!ne1} ] && [ ${!ne1} -eq 1 ]; then
            if [ -e $src ]; then
                qq_exit "$src exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n "${!f2}" ] && [ "${!f2}" -eq 1 ]; then
            if ! [ -f $dist ]; then
                qq_exit "$dist not exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n "${!nf2}" ] && [ "${!nf2}" -eq 1 ]; then
            if [ -f $dist ]; then
                qq_exit "$dist exist file" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n "${!d2}" ] && [ "${!d2}" -eq 1 ]; then
            if ! [ -d $dist ]; then
                qq_exit "$dist not exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n "${!nd2}" ] && [ "${!nd2}" -eq 1 ]; then
            if [ -d $dist ]; then
                qq_exit "$dist exist dir" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!e2} ] && [ ${!e2} -eq 1 ]; then
            if ! [ -e $dist ]; then
                qq_exit "$dist not exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
        ...
        if [ -n ${!ne2} ] && [ ${!ne2} -eq 1 ]; then
            if [ -e $dist ]; then
                qq_exit "$dist exist" "${PATH_MAIN_REPO_FN_DIR}"/dir_"$FNN"/"$FNN".sh "$LINENO"
            fi
        fi
