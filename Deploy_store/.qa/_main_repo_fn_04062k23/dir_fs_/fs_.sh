...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_fs_/fs_.sh"
...
${NORMAL}"
    ...
    # >${FN_OUT_DIR}/com_fs_.res
    # COMMAND &>>${FN_OUT_DIR}/com_fs_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_fs_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_fs_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_fs_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
