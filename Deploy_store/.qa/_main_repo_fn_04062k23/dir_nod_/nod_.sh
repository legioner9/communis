...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_nod_/nod_.sh"
...
MAIN: like exf_ with add nod_.var file in PATH_VAR_DIR/nod_
NAME: ${FNN}()
...
${NORMAL}"
    ...
    # >${FN_OUT_DIR}/com_nod_.res
    # COMMAND &>>${FN_OUT_DIR}/com_nod_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_nod_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_nod_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_nod_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    local arg_arr=("${ARGS[@]:1}")

    plt_nod_=${PATH_VAR_DIR}/nod_/nod_.var
    ...
    if ! is_est_ ${plt_nod_} -f; then
        plt_exit "${plt_nod_} not exist"
        return 1
    fi

    exl_ --list "${plt_nod_}"

    if is_est_ nod_.var -f &>/dev/null; then
        exl_ --list nod_.var
    fi
    ...
    # echo -e "${GREEN}\${arg_arr[@]} = ${arg_arr[*]}${NORMAL}" #print variable

    # local e_args=($(earg2e2_ ${ARGS[@]:1}))

    if [ -n "${arg1}" ]; then
        echo -e "${HLIGHT}--- ${arg1} ---${NORMAL}" #start files "${arg1} ${ARGS[@]:1}"
        eval ${arg1}
    fi
    # ------------------------------------------
    # -----------------------------------------------------------------------------------------
