...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_cr_docman_from_file_/_cr_docman_from_file_/_sh/cr_unix_man.sh"
    ...
    for item in "${_f2a_all_[@]}"; do
        ...
        if [ ! $item == "" ]; then                    # ← see 'man test' for available unary and binary operators.
            ...
            if [ ! -f "$item_dir/$item.man" ]; then
                cp "${PATH_MAIN_REPO_FN_DIR}/dir_cr_docman_from_file_/_cr_docman_from_file_/_tml/doc_file/file_man.tml" "$item_dir/$item.man"
