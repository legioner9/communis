...
f_f_arg_() {
    ...
    *)
        ...
        wrp_fifs1_ cd ${dir_fns_} -d1

        qq_pause "(${name_fn_}) create !!! in ($PWD/dir_${name_fn_}/) file (${name_fn_}.sh) dir (_${name_fn_}) ?" "$filename" "$LINENO"

        mkdir_h_ifnotexsist_h "$PWD/dir_${name_fn_}" "$filename" "$LINENO"

        cp_h "$ORIGO_DIR/_f_f_/_tml/f_f_" "$PWD/dir_${name_fn_}/${name_fn_}.sh" "$filename" "$LINENO"

        cp_h "$ORIGO_DIR/_f_f_/_tml/file.man" "$PWD/dir_${name_fn_}/${name_fn_}.man" "$filename" "$LINENO"
        cp_h "$ORIGO_DIR/_f_f_/_tml/file.lst" "$PWD/dir_${name_fn_}/${name_fn_}.lst" "$filename" "$LINENO"

        cp_h "$ORIGO_DIR/_f_f_/_tml/_00000" "$PWD/dir_${name_fn_}/_${name_fn_}"
        ...
        mv "${PATH_MAIN_REPO_FN_DIR}/dir_${name_fn_}/_${name_fn_}/_config/mane_.tag" "${PATH_MAIN_REPO_FN_DIR}/dir_${name_fn_}/_${name_fn_}/_config/mane_${name_fn_}.tag"
        ...
        wrp_fifs1_ cd ${name_fn_}extst -d
        ...
        cr_tst_dir_ --name "${name_fn_}"tst --ext tst
