...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_exf_/exf_.sh"
...
MAIN: exec function with args_cntl from .var inject file in root dir
NAME: ${FNN}()
WHERE?:(only in root dir)Y/N
WHAT?: only in root dir
ARGS: 
\$1
[ ,\$2 num_menu ]
CNTLS: !!! not working with cntls !!!
required
...
${NORMAL}"
    ...
    # >${FN_OUT_DIR}/com_exf_.res
    # COMMAND &>>${FN_OUT_DIR}/com_exf_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_exf_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_exf_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_exf_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    if ! is_est_ exf_.var -f; then
        plt_exit "exf_.var not exist"
        return 1
    fi

    # exl_ --list exf_.var 1>/dev/null
    exl_ --list exf_.var
