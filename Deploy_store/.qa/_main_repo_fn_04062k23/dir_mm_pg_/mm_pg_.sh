...
filename="${PATH_MAIN_REPO_FN_DIR}/dir_mm_pg_/mm_pg_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_pg_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_pg_
    ...
    # >${FN_OUT_DIR}/com_mm_pg_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_pg_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_pg_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_pg_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_pg_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    mm_pg_access_local() {
        sudo su
        echo -e "${HLIGHT}--- sudo echo \"listen_addresses = 'localhost'\" >>/var/lib/pgsql/data/postgresql.conf ---${NORMAL}" #start files
        sudo echo "listen_addresses = 'localhost'" >>/var/lib/pgsql/data/postgresql.conf
        su -l st
    }

    mm_pg_list_files() {
        echo "Config pg /var/lib/pgsql/data/postgresql.conf"
        echo "Для управления доступом, правьте файл /var/lib/pgsql/data/pg_hba.conf"
    }

    mm_pg_cr_role() {

        echo "enter name role: [role]"
        ques_arg_ createuser -U postgres --no-superuser --no-createdb --no-createrole --encrypted --pwprompt

    }

    mm_pg_cr_db() {
        echo "for create db enter: [role] [db_name]"   
        ques_arg_ createdb -U postgres -O
    }

    mm_pg_echo_all_accessed_bd() {
        echo -e "${HLIGHT}---  psql -U postgres -c \"\l+\" ---${NORMAL}" #start files
        psql -U postgres -c "\l+"
    }
    ...
    res+=("echo bye)))")
    #{arr_res}
    arr+=("install pg15, sysdb")
    res+=("mm_pg_inst_pg15")
    arr+=("start pg_s")
    res+=("sudo service postgresql start")
    arr+=("default start pg_s")
    res+=("sudo chkconfig postgresql on")
    arr+=("! under super user ! network access local")
    res+=("mm_pg_access_local")
    arr+=("list usage pg files")
    res+=("mm_pg_list_files")
    arr+=("go to admin pg")
    res+=("sudo psql -U postgres")
    arr+=("create pg role(user)")
    res+=("mm_pg_cr_role")
    arr+=("create bd (role) (db_name)")
    res+=("mm_pg_cr_db")

    arr+=("echo all accessed bd")
    res+=("mm_pg_echo_all_accessed_bd")
    arr+=("bcp all db to REPOBARE/_repo/NPgsql/pg_bcp/all")
    res+=("pg_dumpall -U postgres -f /home/st/REPOBARE/_repo/NPgsql/pg_bcp/all/posgresql")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
